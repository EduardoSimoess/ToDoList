...
export default function List() {
    ...
    return (
        <div className="font-serif flex flex-col gap-3 border-t-2 border-gray-300 pb-2 pt-3 dark:dark:text-white">
            {list.map((item, i) => (
                <div key={i} className="flex flex-col text-start border-b-2 border-gray-300 pb-2">
                    ...
                    <div className="flex flex-row justify-between gap-1 items-center font-mono">
                        { item.checked ? <div className="text-gray-500 dark:text-stone-200 text-sm font-serif">Done</div> : <div className="text-red text-red-500 dark:text-purple-600 text-sm font-serif">Pending</div>}
                        <FaTrash className="text-gray-500 dark:text-white dark:hover:text-purple-600 hover:text-red-500 h-3" key={`btn${i}`} onClick={() => handleExcludeBtn(i)}>Delete</FaTrash>
                    </div>
                </div>
            ))}
        </div>
